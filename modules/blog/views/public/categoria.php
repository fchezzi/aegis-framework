...
<head>
    <meta charset="UTF-8">
    <title><?= htmlspecialchars($categoria['nome']) ?> - Blog</title>
    <style>
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul>
            <?php foreach ($categorias as $cat): ?>
                <li><a href="<?= url('/blog/' . $cat['slug']) ?>" <?= $cat['id'] === $categoria['id'] ? 'style="font-weight: bold; color: #007bff;"' : '' ?>><?= htmlspecialchars($cat['nome']) ?> (<?= $cat['total_posts'] ?>)</a></li>
            <?php endforeach; ?>
        </ul>
    </div>
    <?php endif; ?>
    ...
    <div class="header">
        ...
        <?php endif; ?>
        <p style="color: #666;"><?= $total ?? 0 ?> post(s) | <a href="<?= url('/blog') ?>">← Voltar ao Blog</a></p>
    </div>
    ...
    <?php if (isset($totalPages) && $totalPages > 1): ?>
        <div class="pagination">
            <?php if ($page > 1): ?>
                <a href="<?= url('/blog/' . $categoria['slug'] . '/pagina/' . ($page - 1)) ?>">← Anterior</a>
            <?php endif; ?>

            <span>Página <?= $page ?> de <?= $totalPages ?></span>

            <?php if ($page < $totalPages): ?>
                <a href="<?= url('/blog/' . $categoria['slug'] . '/pagina/' . ($page + 1)) ?>">Próxima →</a>
            <?php endif; ?>
        </div>
    <?php endif; ?>
</body>
